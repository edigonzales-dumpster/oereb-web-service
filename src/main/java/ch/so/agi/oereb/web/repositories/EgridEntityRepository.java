...
public interface EgridEntityRepository extends JpaRepository<EgridEntity, Long> {
	...
	List<EgridEntity> findByNumberAndIdentdn(@Param("number") String number, @Param("identdn") String identdn);

    @Query(nativeQuery = true)
	List<EgridEntity> findByXY(@Param("point") Point point);
    
    @Query(nativeQuery = true)
	List<EgridEntity> findByGNSS(@Param("point") Point point);
